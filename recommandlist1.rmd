...
#### Recommanding movies for no information users: 
...
rec_list(movie_new,n,10)#recommand 10 movies
...
ggplot(users, aes(x =factor(1), fill = factor(gender)))+geom_bar(width = 1)+ coord_polar(theta = "y")+scale_fill_discrete(guide=guide_legend(title = "Gender"))+ggtitle("The proportion of Gender")+xlab("")+ theme_bw()
ggplot(users, aes(x =occupation, fill =factor(occupation)))+geom_bar()+scale_fill_discrete(guide=guide_legend(title = "Occupation"))+ggtitle("The distribution of Occupation")+ theme_bw()
...
# randomly select 1 user from the user dataset (assume we only have these information):
#### Recommanding movies for   users with personal informations: 
x<- 150 # an example
user_infor<-users[x,]
age<-user_infor$age # age information for this user
sex<-user_infor$gender # gender information for this user
rec_list1<-function(age,sex,movie_new,users,rec){
  agelevel<-quantile(users$age, probs = seq(0, 1, 0.25))
  checkinter<-data.frame(L=agelevel[1:4],H=agelevel[2:5])
checkinter$Int<-cbind( checkinter[,1] <=age & checkinter[,2] >= age )
agebond<-checkinter[checkinter$Int==TRUE,1:2]
users_new<-users %>%
filter(age>=agebond[1,1] & age<=agebond[1,2]) %>%
filter(gender==sex)
n<-dim(users_new)[1]
movie_new1<-movie_new %>%
filter(users %in% users_new$userid)
return(rec_list(movie_new1,n,rec))
}
rec_list1(age,sex,movie_new,users,5)  # If we know user's age and gender.
# [1] Good Will Hunting (1997)         Casablanca (1942)               
# [3] Shawshank Redemption, The (1994) Schindler's List (1993)         
# [5] Wrong Trousers, The (1993)      
rec_list(movie_new,n,5) # If we don't know the age and gender or they are out of range.
